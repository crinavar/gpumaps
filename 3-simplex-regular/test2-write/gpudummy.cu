...
// set device id
#ifdef DEBUG
    printf("gpudummy(): choosing device %i...", D);
    fflush(stdout);
#endif
    ...
    printf("gpudummy(): Cube size is %f MB\n", (float)Vcube * sizeof(char) / (1024.0 * 1024.0f));
#endif

    // allocating unified memory CPU/GPU
    float *h_data, *d_data;
    char *h_outcube, *d_outcube;
    h_data = (float*)malloc(sizeof(float) * N);
    h_outcube = (char*)malloc(sizeof(char) * Vcube);
    ...
    printf("gpudummy(): set outdata init state...");
    fflush(stdout);
#endif
...
#ifdef VERIFY
#ifdef DEBUG
    printf("gpudummy(): verifying data state...");
    fflush(stdout);
#endif
    assert(verify(h_outcube, N, [](char d, float x, float y, float z) { if(d == 0){ return true; } else{return false;} }));
#ifdef DEBUG
    printf("ok\n");
    fflush(stdout);
#endif
...
#ifdef DEBUG
    printf("\x1b[1mgpudummy(): kernel (map=%i, r=%i)...\x1b[0m", maptype, REPEATS);
    fflush(stdout);
#endif
    if (maptype == 0) {
