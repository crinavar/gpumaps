...
__global__ void td_method(float4 *data, int N, float* bdata, int N2){
    ...
    if(k<N2){
        ...
        //int ax = (-(2.0f*(float)N + 1.0f) + sqrt(4.0*(float)N*(float)N - 4.0*(float)N - 8.0*(float)k + 1.0))/(-2.0f);
        //int a = (int)((float)((2.0f*(float)N + 1.0f) - carmack_sqrtf((float)N*(float)N - (float)N - 2.0f*(float)k + 0.25f)));
...
__global__ void td_method_2dr(float4 *a, int N, float* b, int N2){
    ...
    if( threadIdx.y < 2 ){
        ...
        bi = __fmaf_rn(arg, rsqrtf(arg), OFFSET);// + 0.001f;
        bj = bc - (bi*(bi+1) >> 1);
    }
    __syncthreads();
    */
