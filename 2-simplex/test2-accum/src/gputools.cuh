...
void gen_hadouken_pspace_lower_tensor_core(const unsigned int n, dim3& block, dim3& grid, unsigned int* aux1, unsigned int* aux2, unsigned int* aux3) {
    // lower pow, tetrahedron covering (simplex + lower block)
    int nlow = 1 << ((int)floor(log2f(n)));
    block = dim3(SUBBLOCK_SIZE, SUBBLOCK_SIZE, 1);
    int nbo = (n + block.x - 1) / block.x;
    int nb = (nlow + block.x - 1) / block.x;
    int gx = nb <= 1 ? nb + 1 : nb;
    int gy = nbo;
    ...
    *aux3 = gy + extraby + 1;
     ...
     */
#ifdef DEBUG
    printf("[lower] Sub Block= %i x %i x %i    grid = %i x %i x %i\n", block.x, block.y, block.z, grid.x, grid.y, grid.z);
#endif
}

// covering from greater simplex
void gen_hadouken_pspace_upper_tensor_core(const unsigned int n, dim3& block, dim3& grid, unsigned int* aux1, unsigned int* aux2, unsigned int* aux3) {
    block = dim3(SUBBLOCK_SIZE, SUBBLOCK_SIZE, 1);
    int nu = 1 << ((int)ceil(log2f(n)));
    int nb = (nu + block.x - 1) / block.x;
    int gx = nb <= 1 ? nb + 1 : nb;
...
unsigned int count_recursions_tensor_cores(unsigned int n, int bsize) {
    unsigned int lpow = 1 << ((unsigned int)floor(log2f(n)));
    unsigned int hpow = 1 << ((unsigned int)ceil(log2f(n)));
    unsigned int nh = n;
    unsigned int numrec = 0;
    do {
        if (hpow - nh < HADO_TOL / BSIZE2D * SUBBLOCK_SIZE) {
            nh = 0;
        } else {
            nh = nh - lpow;
            lpow = 1 << ((unsigned int)floor(log2f(nh)));
...
void create_grids_streams_tensor_core(unsigned int n, unsigned int numrec, dim3* grids, dim3 block, unsigned int* aux1, unsigned int* aux2, unsigned int* aux3, cudaStream_t* streams, unsigned int* offsets) {
    unsigned int lpow = 1 << ((unsigned int)floor(log2f(n)));
    unsigned int hpow = 1 << ((unsigned int)ceil(log2f(n)));
    unsigned int off = 0;
    unsigned int nh = n;
    ...
    do {
        if (hpow - nh < HADO_TOL / BSIZE2D * SUBBLOCK_SIZE) {
            // case n is close to the next power of two, we do all
            gen_hadouken_pspace_upper_tensor_core(nh, block, grids[nr], &aux1[nr], &aux2[nr], &aux3[nr]);
            offsets[nr] = off;
            nh = 0;
        } else {
            // case n is far from its next power of two, we continue in halves
            gen_hadouken_pspace_lower_tensor_core(nh, block, grids[nr], &aux1[nr], &aux2[nr], &aux3[nr]);
            offsets[nr] = off;
            nh = nh - lpow;
            off += lpow;
            lpow = 1 << ((unsigned int)floor(log2f(nh)));
...
template <typename Lambda>
double benchmark_map_hadouken_tensor_core(const int REPEATS, dim3 block, unsigned int n, unsigned int msize, unsigned int trisize, DTYPE* ddata, MTYPE* dmat, Lambda map) {
    cudaEvent_t start, stop;
    cudaEventCreate(&start);
    cudaEventCreate(&stop);
    float time = 0.0;
    unsigned int numrec = count_recursions_tensor_cores(n, SUBBLOCK_SIZE);
    dim3* grids = (dim3*)malloc(sizeof(dim3) * numrec);
    unsigned int* subBlockGridSizex = (unsigned int*)malloc(sizeof(unsigned int) * numrec);
    unsigned int* subBlockGridSizey = (unsigned int*)malloc(sizeof(unsigned int) * numrec);
    unsigned int* offsets = (unsigned int*)malloc(sizeof(unsigned int) * numrec);
    unsigned int* auxs1 = (unsigned int*)malloc(sizeof(unsigned int) * numrec);
    unsigned int* auxs2 = (unsigned int*)malloc(sizeof(unsigned int) * numrec);
    unsigned int* auxs3 = (unsigned int*)malloc(sizeof(unsigned int) * numrec);
...
#ifdef DEBUG
    printf("[Hadouken Tensor Core] Recursions number: %i\n", numrec);
#endif

    create_grids_streams_tensor_core(n, numrec, grids, block, auxs1, auxs2, auxs3, streams, offsets);

    // TWEAK EVERY GRID SIZE TO ACCOUNT FOR SUBBLOCKS
    for (int i = 0; i < numrec; i++) {
        subBlockGridSizex[i] = grids[i].x;
        subBlockGridSizey[i] = grids[i].y;
        grids[i].x = (grids[i].x * SUBBLOCK_SIZE + BSIZE2D - 1) / BSIZE2D;
        grids[i].y = (grids[i].y * SUBBLOCK_SIZE + BSIZE2D - 1) / BSIZE2D;
#ifdef DEBUG
        printf("[Hadouken Tensor Core] After grid size tweak, the new grid is: GRID = (%i, %i, %i)\n", grids[i].x, grids[i].y, grids[i].z);
#endif
    }

#ifdef DEBUG
    printf("[Hadouken Tensor Core] HADO_TOL = %i\n", HADO_TOL / BSIZE2D * SUBBLOCK_SIZE);
    print_grids_offsets(numrec, grids, block, offsets);
#endif
    // printf("n = %i    lpow = %i, hpow = %i\n", n, lpow, hpow);
    // printf("numrecs = %i\n", numrec);
...
#pragma loop unroll
    for (int k = 0; k < REPEATS; ++k) {
        for (int i = 0; i < numrec; ++i) {
            kernel_test_tensor_core<<<grids[i], block, 0, streams[i]>>>(n, 1, msize, ddata, dmat, map, auxs1[i], auxs2[i], offsets[i], subBlockGridSizex[i], subBlockGridSizey[i]);
        }
        cudaDeviceSynchronize();
    }
    cudaEventRecord(stop, 0);
