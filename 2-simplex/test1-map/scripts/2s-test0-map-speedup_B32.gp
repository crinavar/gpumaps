...
set output 'plots/2s-test0-zerowork-speedup-B32.eps'
set title '2s-zero-work Kernel Speedup, Titan X, {/Symbol r}=32'
set ytics mirror
unset ytics
...
plot    fbb(x) notitle dashtype 2 lc rgb "black",\
        ...
        'data/2s-test0-map-titanx_B32.dat' using 1:($3/$23) title "{/Symbol l}({/Symbol w})" with linespoints ls 1,\
        'data/2s-test0-map-titanx_B32-recursive.dat' using 1:($3/$31) title "Recursive" with linespoints ls 3,\
        'data/2s-test0-map-titanx_B32.dat' using 1:($3/$7) title "UTM" with linespoints ls 4
