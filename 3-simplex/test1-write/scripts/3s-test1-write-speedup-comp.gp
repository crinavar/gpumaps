...
#set format y2 "10^{%L}"
#set format x "2^{%L}"
set key Left right bottom reverse samplen 1.0 font "Times,20" spacing 1
...
fbb(x)      =1
titanx(x)   =1.6 - b1*(c1/x)**d1
gtx1050ti(x)=1.5 - b2*(c2/x)**d2
#teslak40(x) =1.0 - b3*(c3/x)**d3
teslak40(x) = b3*c3*log(x)**d3
teslav100(x)=1.5 - b4*(c4/x)**d4

fit titanx(x) 'data/3s-test1-write-titanx_B8.dat' u 1:($3/$7) via b1,c1,d1
fit gtx1050ti(x) 'data/3s-test1-write-titanx_B8.dat' u 1:($3/$7) via b2,c2,d2
fit teslak40(x) 'data/3s-test1-write-titanx_B8.dat' u 1:($3/$7) via b3,c3,d3
fit teslav100(x) 'data/3s-test1-write-titanx_B8.dat' u 1:($3/$7) via b4,c4,d4

set pointsize   0.7
#plot    fbb(x) notitle dashtype 2 lc rgb "black",\
#        'data/3s-test1-write-titanx_B8.dat' u 1:($3/$7) title "Titan X"       with points ls 1,   titanx(x) notitle ls 9,\
#        'data/3s-test1-write-titanx_B8.dat' u 1:($3/$7) title "GTX 1050 Ti"   with points ls 2,   gtx1050ti(x) notitle ls 10,\
#        'data/3s-test1-write-teslak40_B8.dat' u 1:($3/$7) title "Tesla K40"     with points ls 3,   teslak40(x) notitle ls 11,\
#        'data/3s-test1-write-titanx_B8.dat' u 1:($3/$7) title "Tesla V100"    with points ls 4,   teslav100(x) notitle ls 12
...
plot    fbb(x) notitle dashtype 2 lc rgb "black",\
        ...
        'data/3s-test1-write-titanx_B8.dat' u 1:($3/$7) title "Tesla V100"    with linespoints ls 4
