...
statistics gpudummy(unsigned int method, unsigned int repeats){
...
#ifdef DEBUG
    printf("Allocating Compressed memory Spaces of %lu x %lu = %lu -> %f MB in CPU & GPU.......", cn, cm, cn*cm, cn*cm*sizeof(MTYPE)/(1024.0*1024.0)); fflush(stdout); 
#endif
    Cm_h = (MTYPE*)malloc(sizeof(MTYPE)*cm*cn);

    cudaMalloc(&Ec_d, sizeof(MTYPE)*n*n);
    cudaMalloc(&Cm_d, sizeof(MTYPE)*cn*cm);
    ...
    initmat<MTYPE>(Cm_h, cn*cm, 0); 
#ifdef DEBUG
    printf("ok\n"); fflush(stdout);
#endif
    cudaMemcpy(Ec_d, Ec_h, sizeof(MTYPE)*n*n, cudaMemcpyHostToDevice);
    cudaMemcpy(Cm_d, Cm_h, sizeof(MTYPE)*cn*cm, cudaMemcpyHostToDevice);
    ...
    assert( verify<MTYPE>(Ec_h, n, 1) ); // Verifying results
    ...
    if (!isBB)
        for(int i=0; i<REPEATS; ++i){
            ...
            for(int i=0; i<INNER_REPEATS; ++i){
                kernel_write_lambda<<< grid, block >>>(M, n, nb, rb, 1, WSIZE, map); //TODO: fix
        ...
        for(int i=0; i<REPEATS; ++i){
            cudaEventRecord(start);
            for(int i=0; i<INNER_REPEATS; ++i){
                kernel_write<<< grid, block >>>(M, n, nb, rb, 1, WSIZE, map); //TODO: fix
                cudaDeviceSynchronize();
            }
            last_cuda_error("lastError");
            cudaDeviceSynchronize();
